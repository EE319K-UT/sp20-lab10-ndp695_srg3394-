...
void ADC_Init(void){ 
	...
	GPIO_PORTD_DIR_R &= ~0x02; //make PD2 input
	GPIO_PORTD_AFSEL_R |= 0x02; //enable alternate fun on PD2
	GPIO_PORTD_DEN_R &= ~0x02; //disable digital I/O on PD2
	GPIO_PORTD_AMSEL_R |= 0x02; //enable analog fun on PD2
	...
	ADC0_SSMUX3_R = (ADC0_SSMUX3_R & 0xFFFFFFF0) + 5; //connects channel 6 to ADC
